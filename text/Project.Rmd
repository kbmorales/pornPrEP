...
use if statement to check in packages are installed and if not install them
...
packages = c("devtools", "rmarkdown", "httr", "knitr", "rvest", "stringr", "rebus", "dplyr", "lubridate", "moments", "cowplot", "ggplot2", "wordcloud", "ggthemes", "kableExtra", "tidyr", "RColorBrewer", "reshape2", "quantreg", "splines")
...
cutoff <- 2014.5 - min(cleandata$year)
...
`r plot(rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "Yes", tau = 2:98/100), mfrow = c(1,3))`
...
bbyKTestLS <- KhmaladzeTest(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "Yes", taus = -1, nullH = "location-scale", trim = c(0.05, 0.95))
bbyKTestL <- KhmaladzeTest(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "Yes", taus = -1, nullH = "location")
bbnKTestLS <- KhmaladzeTest(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "No", taus = -1, nullH = "location-scale", trim = c(0.05, 0.95))
bbnKTestL <- KhmaladzeTest(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "No", taus = -1, nullH = "location")
```

Based on this analysis, and excluding the two most extreme quantiles in (0.05, 0.95), the joint test statistic for the model testing location-scale is `r KTestLS$Tn` and for location is `r KTestL$Tn`. The asymptotic critical values at 1% is 5.350 4.523
4.091 (see [Koenker and Xiao (2001)](http://www.econ.uiuc.edu/~roger/research/inference/khmal6ap.pdf). Thus, we assume the model specification is of the location-scale shift form. Notably, the bareback categorization remains highly significant under both null hypotheses (`r KTestLS$THn[1]` and `r KTestLS$THn[2]`).

Given the location-scale shift formulation, an analysis which utilizes the a shift in scale, or slope, of the quantile regression will be examined next. 

#### Analysis 2 - Splined Quantile Regression, by Bareback classification 
...
bbmed <- rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "Yes", tau = 0.5)
notbbmed <- rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "No", tau = 0.5)
taus <- c(.25,.75)
quantbb <- rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "Yes", tau = taus)
quantnotbb <- rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "No", tau = taus)

mrbby <- summary(rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "Yes", tau = 2:98/100))
mrbbn <- summary(rq(logpviews ~ bs(phyear, degree=1, knots= cutoff), subset = bareback == "No", tau = 2:98/100))
par(yaxs = "i")
plot(mrbby, mfrow = c(1,3))
plot(mrbbn, mfrow = c(1,3))

regplot <- ggplot(cleandata, aes(x = phyear, y = logpviews)) + scale_x_continuous(breaks = unique(phyear), labels=as.character(unique(year))) + theme_minimal() + theme(legend.position = "bottom") + scale_color_brewer(palette="Paired")
...
regplot_2 <- regplot + 
  ...
  geom_segment(aes(x = 0, y = notbbmed$coefficients[1], xend = 4.5, yend =  notbbmed$coefficients[1] + notbbmed$coefficients[2]), size = 1) + 
  geom_segment(aes(x = 4.5, y = notbbmed$coefficients[1] + notbbmed$coefficients[2], xend = 7, yend = notbbmed$coefficients[1] + notbbmed$coefficients[3]), size = 1) + 
  geom_segment(aes(x = 0, y = bbmed$coefficients[1], xend = 4.5, yend = bbmed$coefficients[1] + bbmed$coefficients[2]), linetype = "dashed", size = 1, color = "red") + 
  geom_segment(aes(x = 4.5, y = bbmed$coefficients[1] + bbmed$coefficients[2], xend = 7, yend = bbmed$coefficients[1] + bbmed$coefficients[3]), linetype = "dashed", size = 1, color = "red") +
  geom_vline(xintercept = 4.5, linetype = "dotted")
regplot_2
```

Equality of slopes between both fits
...
quantiles <- rq(logpviews ~ phyear + bareback, tau = taus)
